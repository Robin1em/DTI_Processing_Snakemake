...
def generate_config():
    ...
    after_AP = [x.split("_AP")[1] for x in niftis]
    after_AP_before_nii = [x.split(".nii")[0] for x in after_AP]
    b_niftis = [re.sub(r"_AP.*?.nii", "", x.replace("origs/", "").replace(".gz", "")) for x in niftis if not "_b0_" in x]
    ...
    PA_names = [re.sub(r"_AP.*?.nii", "", x.replace("origs/", "").replace(".gz", "")) for x in PA_niftis]
    jsons = glob("origs/*AP*json")
    bvecs = glob("origs/*.bvec")
    AP_b0s = [f for f in glob("*b0*.nii.gz") if not "PA" in f]
    return {"sample_ids": sample_ids, "b_values": b_values, "niftis": niftis, "shortbvals": shortbvals, "PA_niftis": PA_niftis, "PA_names": PA_names, "jsons": jsons, "bvecs": bvecs, "AP_b0s": AP_b0s, "after_AP": after_AP, "name_end": after_AP_before_nii}
...
endings = [".bvec", ".bval"]
bvec_files_cut = [re.sub(r"_AP.*?.bvec", "", x.replace("origs/", "")) for x in conf["bvecs"]]
samples = bvec_files_cut

after_AP = conf["after_AP"][0] # everything after "_AP"
name_end = conf["name_end"][0] # everything between "_AP" and ".nii"
...
rule extract_roi:
    input:
        lambda wc: "origs/" + conf["sample_ids"][conf["b_values"].index(wc.number)] + f"_DTI_{wc.number}" + "_AP" + after_AP
    output:
